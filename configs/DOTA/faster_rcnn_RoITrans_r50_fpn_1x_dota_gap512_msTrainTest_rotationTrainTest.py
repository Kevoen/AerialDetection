# model settings
model = dict(
    type='RoITransformer',                           # model类型
    pretrained='modelzoo://resnet50',                # 预训练模型：resnet50
    backbone=dict(                                   # 
    ...
    rpn_head=dict(
        ...
        loss_cls=dict(
            type='CrossEntropyLoss', use_sigmoid=True, loss_weight=1.0),            # ？？  计算损失loss
        loss_bbox=dict(type='SmoothL1Loss', beta=1.0 / 9.0, loss_weight=1.0)),
    bbox_roi_extractor=dict(
        type='SingleRoIExtractor',                  # 单个RoI提取器类型
    ...
    bbox_head=dict(
        type='SharedFCBBoxHeadRbbox',               # 全连接层类型
        num_fcs=2,                                  # 全连接层数量
        in_channels=256,                            # 输入通道数
        fc_out_channels=1024,                       # 输出通道数
        roi_feat_size=7,                            # ROI特征层尺寸
        num_classes=16,                             # 分类器的类别数量+1，+1是因为多了一个背景的类别
        target_means=[0., 0., 0., 0., 0.],          # 均值
        target_stds=[0.1, 0.1, 0.2, 0.2, 0.1],      # 方差
    ...
    rbbox_roi_extractor=dict(
        type='RboxSingleRoIExtractor',              # RRoI提取器类型
        roi_layer=dict(type='RoIAlignRotated', out_size=7, sample_num=2),   # ROI具体参数：RROI类型为ROIalign，输出尺寸为7，sample数为2
        out_channels=256,                           # 输出通道数
        featmap_strides=[4, 8, 16, 32]),            # 特征图的步长
    rbbox_head = dict(
...
train_cfg = dict(
    rpn=dict(
        assigner=dict(
            type='MaxIoUAssignerCy',    # RPN网络的正负样本划分
    ...
    rcnn=[
        dict(
            assigner=dict(
                type='MaxIoUAssignerCy',     # RCNN网络正负样本划分
                pos_iou_thr=0.5,
                neg_iou_thr=0.5,
                min_pos_iou=0.5,
                ignore_iof_thr=-1),
...
test_cfg = dict(
    ...
    rcnn=dict(
        ...
        score_thr = 0.05, nms = dict(type='py_cpu_nms_poly_fast', iou_thr=0.1), max_per_img = 2000)  # max_per_img表示最终输出的det bbox数量
...
# dataset settings
dataset_type = 'DOTADataset'          # 数据集类型
# data_root = 'data/dota1_1024/'
data_root = 'data/dota1_1024_v2/'     # 数据集根目录
img_norm_cfg = dict(
...
data = dict(
    ...
    train=dict(
        ...
        ann_file=[data_root + 'trainval1024/DOTA_trainval1024.json',                 # 数据集annotation路径
                  data_root + 'trainval1024_ms/DOTA_trainval1024_ms.json'],
        img_prefix=[data_root + 'trainval1024/images/',                              # 数据集的图片路径
                    data_root + 'trainval1024_ms/images/'],
        img_scale=(1024, 1024),                                                      # 输入图像尺寸，最大边1024，最小边1024
        ...
        rotate_aug=dict(border_value=0, small_filter=6)                              # 旋转参数
    ),
    val=dict(
        type=dataset_type,
...
checkpoint_config = dict(interval=12)               # 每12个epoch存储一次模型
# yapf:disable
log_config = dict(
...
work_dir = './work_dirs/faster_rcnn_RoITrans_r50_fpn_1x_dota_gap512_msTrainTest_rotationTrainTest'  # log文件和模型文件存储路径
